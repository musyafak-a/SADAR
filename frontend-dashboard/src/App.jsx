...
import { Camera, AlertTriangle, ShieldCheck, Clock, Settings, Home, BarChart2, Box, Video, User } from 'lucide-react';
...
export default function App() {
  ...
  return (
    <div className="min-h-screen bg-slate-50 font-sans text-slate-800 flex overflow-hidden">
      ...
      {/* Sidebar */}
      <aside className="w-[260px] bg-[#111827] text-slate-300 flex flex-col shrink-0 h-screen z-20">
        <div className="p-6 border-b border-slate-800 bg-[#0f1522]">
        ...
        <div className="p-5 border-t border-slate-800 bg-[#0f1522]">
          <div className="flex items-center gap-3">
            ...
            <div className="text-left">
              <p className="text-sm font-bold text-slate-200">Ajinomoto</p>
              <p className="text-xs text-slate-500">Mojokerto</p>
            </div>
          </div>
        </div>
      </aside>
      ...
      <div className="flex-1 flex flex-col h-screen overflow-hidden relative z-10">
        ...
        <header className="h-[72px] bg-white border-b border-slate-200 px-8 flex items-center justify-between shrink-0 shadow-sm">
          <div>
            <h2 className="text-xl font-bold text-slate-800">Live Monitoring CCTV</h2>
            <p className="text-sm text-slate-500">Pantau kondisi lapangan secara real-time dari berbagai titik kamera.</p>
          </div>
          
          <div className="flex items-center gap-8">
            <div className="flex items-center gap-3 text-sm text-slate-500 font-medium bg-slate-50 px-4 py-2 rounded-lg border border-slate-100">
        ...
        {/* Dashboard Content */}
        <main className="flex-1 p-8 flex gap-6 overflow-hidden bg-slate-50/50">
          
          {/* Left Column: Live Feed */}
          <div className="flex-1 flex flex-col gap-6 overflow-hidden">
            
            {/* Video Player */}
            <div className="card-panel flex-1 relative overflow-hidden flex flex-col bg-slate-900 border-0 shadow-md">
              <div className="absolute top-4 left-4 z-10 flex items-center gap-2">
                <span className={`px-3 py-1.5 rounded-md bg-black/60 backdrop-blur-md text-xs font-bold text-white border border-white/10 flex items-center gap-2`}>
                  <Video className="w-4 h-4 text-slate-300" />
                  {activeCam?.name}
                </span>
                <span className={`px-2.5 py-1.5 rounded-md text-[10px] font-bold uppercase tracking-widest flex items-center gap-1.5 ${isLive ? 'bg-green-600/90 text-white' : 'bg-slate-700/90 text-white'}`}>
                  {isLive ? 'LIVE' : 'OFFLINE'}
                </span>
              </div>
              ...
              <div className="flex-1 relative flex items-center justify-center overflow-hidden">
                ...
                <div className={`absolute inset-0 flex-col items-center justify-center text-slate-500 gap-4 ${isLive ? 'hidden' : 'flex'} bg-slate-900`}>
                  ...
                  <p className="font-mono text-sm tracking-widest uppercase">Stream Offline</p>
                </div>
              </div>
            </div>

            {/* Camera Selection Grid */}
            <div className="h-[140px] shrink-0 grid grid-cols-4 gap-4">
              {cameras.map((cam, idx) => (
                <button
                  key={cam.id}
                  onClick={() => setSelectedCamera(cam.id)}
                  className={`relative p-4 rounded-xl text-left transition-all duration-200 border bg-white flex flex-col justify-between ${
                ...
                >
                  <div className="flex justify-between items-start w-full">
                    ...
                    <div className={`px-2 py-0.5 rounded text-[10px] font-bold uppercase ${cam.status === 'active' ? 'bg-green-100 text-green-700' : 'bg-slate-100 text-slate-500'}`}>
                      {cam.status === 'active' ? 'LIVE' : 'OFF'}
                    </div>
                  </div>
                  <div className="mt-4">
                    <h3 className={`font-bold text-sm truncate ${selectedCamera === cam.id ? 'text-[#E31837]' : 'text-slate-800'}`}>{cam.name}</h3>
          ...
          <div className="w-[400px] shrink-0 card-panel flex flex-col overflow-hidden">
            <div className="p-5 border-b border-slate-100 flex justify-between items-center bg-white">
              <h2 className="font-bold text-slate-800 text-lg">Live Alert Log</h2>
              <div className="flex items-center gap-1.5 px-2.5 py-1 rounded-full bg-green-50 border border-green-100">
                <span className="w-1.5 h-1.5 rounded-full bg-green-500 animate-pulse"></span>
                <span className="text-[10px] font-bold text-green-700 uppercase tracking-wide">Real-time</span>
              </div>
            </div>
              ...
              {alerts.map((alert, idx) => (
                <div key={alert.id} className="flex gap-4 group relative">
                  ...
                  <div className="relative z-10 shrink-0 mt-1">
                    <div className={`w-8 h-8 rounded-full flex items-center justify-center border shadow-sm ${getSeverityColors(alert.severity)}`}>
                  ...
                  <div className="flex-1 pb-1">
                    <div className="flex items-center gap-3 mb-1">
                      <span className="text-xs font-bold text-slate-500 bg-white px-2 py-0.5 rounded border border-slate-200 shadow-sm">{alert.time}</span>
                      <h3 className="text-sm font-bold text-slate-800 leading-none">{alert.type}</h3>
                    </div>
                    <p className="text-xs text-slate-500 mt-1.5 ml-1">{alert.camera}</p>
                  </div>
                </div>
              ))}
            </div>
            ...
            <div className="p-4 border-t border-slate-100 bg-white text-center">
              <button className="text-sm font-bold text-blue-600 hover:text-blue-700 hover:underline transition-colors">Lihat semua alert →</button>
            </div>
          </div>
        </main>
      </div>
